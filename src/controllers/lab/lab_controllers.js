...
const CreateLab = async (req, res) => {
  ...
  if (
    getUserByID.userData.CampusAdminLevel === 1 ||
    getUserByID.userData.tipo === 1
  ) {
    return res.status(400).json({
      status: false,
  ...
  const GetCampusByID = await instituteModels.getCampusByID(
    getUserByID.userData.ID_campus
  );
  ...
  const CreateLab = await labModels.CreateLab(
    sala,
    capacity,
    getUserByID.userData.ID_campus
  );
